...
LINE_COLOR = (0, 0, 0)
OCCLUSION_TIME = .5 #.5 seconds for occlusion
# ==============================================================================
# -- World --------NO NEED-----------------------------------------
# ==============================================================================
...
class DualControl(object):
    def __init__(self ):
        ...
        self.overlay_button = 0
        self.overlay_button_time = time.time()
        self.pause_button_idx = 8
        self.pause_button = 0
    def _parse_vehicle_wheel(self, event):
        #self.button_press = 0
        pygame.joystick.init()
        ...
        steerCmd = jsInputs[self._steer_idx] * 450
        ...
        if (self.overlay_button == 1 and time.time() - self.overlay_button_time >= OCCLUSION_TIME ):
            self.overlay_button = 0

        #toggle = jsButtons[self._reverse_idx]
        for e in event:
            if e.type == pygame.JOYBUTTONDOWN:
                if e.button == self._overlay_button_idx:
                    self.overlay_button = 1
                    self.overlay_button_time = time.time()
                if e.button == self.pause_button_idx:
...
def game_loop(args):
    ...
    # writes csv file
    filename = "plugin_data.csv"
    path = pathlib.Path(filename)
    path.unlink(missing_ok = True)
    # don't know if we make the file here
    csvfile = open(filename, 'a', newline='', encoding='utf-8')
    c = csv.writer(csvfile)
    c.writerow( ['time','wheel_degree','throttle', 'brake', 'velocity_x', 'velocity_y', 
                 'velocity_z', 'speed', 'acceleration_x', 'acceleration_y', 'acceleration_z',
                 'occlusion'] )
    ...
    try:
        ...
        while running:
            ...
            controller._parse_vehicle_wheel(event)
         ...
         #   print("Car Acceleration: ", acceleration)
            # print(controller.pause_button)
            while controller.pause_button == 1 and running:
                print('IN LOOP')
                event = pygame.event.get()
                controller._parse_vehicle_wheel(event)
                for e in event:
            ...
            update_data(data6, [controller.overlay_button])

            display_subscreen(screen, data1, [(0, 255, 0)], 30, 0, "time (s)", "Steer Angle",-450,450,)
            ...
            #save data to csv
            c.writerow([np.round(time.time() - start_time, 2), np.round(controller.steer,2), np.round(controller.throttle,2), np.round(controller.brake,2), np.round(velocity.x,2), 
                        np.round(velocity.y,2), np.round(velocity.z,2), np.round(speed,2), np.round(acceleration.x,2), np.round(acceleration.y,2), np.round(acceleration.z,2), np.round(controller.overlay_button,2)])
