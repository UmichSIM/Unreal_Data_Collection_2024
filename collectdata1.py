...
def draw_labels(screen,last_value):
    font = pygame.font.Font(None, 36)
    
    # X label
    x_label = font.render("Time (s)", True, LINE_COLOR)
    x_label_rect = x_label.get_rect()
    x_label_rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT - 30)
    ...
    y_label_rect.center = (10, GRAPH_Y + GRAPH_HEIGHT // 2)
    ...
    for i in range(-1,8,1):
        ...
        y_axis_value_rect.center = (GRAPH_X - 10, GRAPH_Y + (1 - 10*i / 100) * GRAPH_HEIGHT - 50)
        screen.blit(y_axis_value, y_axis_value_rect)
    ...
    last_value_rect.midleft = (GRAPH_X + GRAPH_WIDTH + 10, GRAPH_Y + (1 - last_value / 100) * GRAPH_HEIGHT - 50)
    screen.blit(last_value_label, last_value_rect)
    
    return x_label, x_label_rect, y_label, y_label_rect, y_axis_value, y_axis_value_rect, last_value_label, last_value_rect
...
def draw_line(graph_surface,data,color ):
    ...
    if len(data) > 1:
        for i in range(len(data) - 1):
            x1 = i * (GRAPH_WIDTH // (MAX_DATA_POINTS - 1))
            y1 = GRAPH_HEIGHT - (data[i] * (GRAPH_HEIGHT / 100)) - 50
            x2 = (i + 1) * (GRAPH_WIDTH // (MAX_DATA_POINTS - 1))
            y2 = GRAPH_HEIGHT - (data[i + 1] * (GRAPH_HEIGHT / 100)) - 50
            pygame.draw.line(graph_surface, color, (x1, y1), (x2, y2), 2)

def draw_graph(screen, speed_data, velocity_x_data):
    graph_surface = pygame.Surface((GRAPH_WIDTH, GRAPH_HEIGHT))
    graph_surface.fill(BACKGROUND_COLOR)
    
    # Draw grid lines
    for i in range(0, GRAPH_HEIGHT, 20):
        pygame.draw.line(graph_surface, (200, 200, 200), (0, i), (GRAPH_WIDTH, i), 1)
...
def draw_graph2(screen, datas, colors):
    graph_surface = pygame.Surface((GRAPH_WIDTH, GRAPH_HEIGHT))
    graph_surface.fill(BACKGROUND_COLOR)
    ...
    for i, data in enumerate(datas):
        draw_line(graph_surface, data, colors[i])
    screen.blit(graph_surface, (GRAPH_X, GRAPH_Y))
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            draw_graph2(screen, datas, colors)
            # Draw labels and the last value
            x_label, x_label_rect, y_label, y_label_rect, y_axis_value, y_axis_value_rect, last_value_label, last_value_rect = draw_labels(screen, last_value)
            screen.blit(x_label, x_label_rect)
            screen.blit(y_label, y_label_rect)
            screen.blit(y_axis_value, y_axis_value_rect)
            if last_value_label is not None:
